...
namespace AspDotNetIdentityOwinDemoApp.Controllers
{
    ...
    {
        //public UserManager<IdentityUser> UserManager { get { return HttpContext.GetOwinContext().Get<UserManager<IdentityUser>>(); } }
        //public SignInManager<IdentityUser, string> SignInManager { get { return HttpContext.GetOwinContext().Get<SignInManager<IdentityUser, string>>(); } }
        ...
        {
            var signInResult = await SignInManager.PasswordSignInAsync(model.Username, model.Password, isPersistent: true, shouldLockout: true);
            switch (signInResult)
            {
        ...
        public async Task<ActionResult> Register(RegisterModel model)
        {
            ...
            if (identityUser != null)
            {
                if (!identityUser.EmailConfirmed)
                {
                    ...
                    await UserManager.SendEmailAsync(identityUser.Id, "Email Confirmation", String.Format("Use link to confirm email: {0}", confirmUrl));
                }
                return RedirectToAction("Index", "Home");
            }
            ...
            if (identityResult.Succeeded)
            {
                ...
                await UserManager.SendEmailAsync(user.Id, "Email Confirmation", String.Format("Use link to confirm email: {0}", confirmUrl));
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordModel model)
        {
            ...
            if (user != null)
            {
                ...
                await UserManager.SendEmailAsync(user.Id, "Password Reset", String.Format("Use link to reset password: {0}", resetUrl));
            }
            return RedirectToAction("Index", "Home");
        }
        ...
        {
            var signInStatus = await SignInManager.TwoFactorSignInAsync(model.Provider, model.Code, true, true);
            switch (signInStatus)
            {
                case SignInStatus.Success:
                    return RedirectToAction("Index", "Home");
